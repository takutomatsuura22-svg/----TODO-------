-- ============================================
-- テスト用志望校マスタデータ（SECURITY DEFINER版）
-- ============================================
-- このSQLをSupabase SQL Editorで実行して、テスト用の志望校マスタを作成します
-- SECURITY DEFINER関数を使用してRLSをバイパスします

-- 一時的なINSERT関数を作成（SECURITY DEFINERでRLSをバイパス）
CREATE OR REPLACE FUNCTION public.insert_test_admission_programs()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
-- 関数を実行
SELECT public.insert_test_admission_programs();

-- 関数を削除（セキュリティのため）
DROP FUNCTION IF EXISTS public.insert_test_admission_programs();
